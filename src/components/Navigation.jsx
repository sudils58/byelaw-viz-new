...
export default function Navigation({ theme, setTheme }) {
    ...
    return (
        <header className="fixed top-8 left-0 right-0 z-50 pointer-events-none px-6 flex justify-center">
            {/* The actual floating bar */}
            <div
                ref={navRef}
                className="pointer-events-auto glass-header px-12 py-5 rounded-full flex items-center justify-between w-full max-w-[1400px] transition-transform duration-700 ease-out"
            >
                {/* Logo & Title */}
                <div className="flex items-center gap-4 w-[400px]">
                    <div className="w-16 h-16 flex items-center justify-center drop-shadow-[0_0_15px_rgba(56,189,248,0.5)]">
                    ...
                    <h1 className="text-xl font-bold text-white tracking-tight leading-tight hidden sm:block">
                        Building Bye Laws <br />
                        <span className="text-indigo-400 font-semibold text-lg">Compliance Calculator</span>
                    </h1>
                </div>

                {/* Navigation Links */}
                <nav className="hidden lg:flex items-center gap-10 text-lg font-medium text-white/70">
                    <button onClick={() => {
                    ...
                    }} className="text-white font-semibold drop-shadow-[0_0_10px_rgba(255,255,255,0.3)] bg-white/10 px-6 py-2.5 rounded-full border border-white/10 cursor-pointer focus:outline-none">Check Compliance</button>

                    <button onClick={() => {
                        document.getElementById('guide')?.scrollIntoView({ behavior: 'smooth' });
                    }} className="hover:text-white transition-colors cursor-pointer focus:outline-none">Permit Guide</button>

                    <button onClick={() => {
                        document.getElementById('resources')?.scrollIntoView({ behavior: 'smooth' });
                    }} className="hover:text-white transition-colors cursor-pointer focus:outline-none">Resources</button>
                ...
                <div className="flex justify-end w-[200px]">
                    <button
                        ...
                        className="w-14 h-14 rounded-2xl flex items-center justify-center text-white/70 hover:text-white transition-all bg-white/5 hover:bg-white/10 border border-white/10"
